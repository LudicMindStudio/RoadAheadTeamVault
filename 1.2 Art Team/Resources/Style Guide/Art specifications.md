...
VERY IMPORTANT:
When creating pixel art, make sure to save your file as a png. Other file types can work, but most importantly do not save them as a jpeg. Doing so results in the image beings permanently changed due to the way jpeg compression works.

\*add example

Art style
-Smooth, big shapes, round, colorful
-Cozy, relaxed.
-
\*perspective guide
...
When making character sheets for characters:

For significant characters:
- Front view of the character's entire body
- Profile view of the character's entire body
- Colour palette
- The character's face + 4 facial expressions (Happy, Surprised, sad, Angry)
- Coloured
...
Environments
- Coloured
- Provide general overview of environment
...
Objects
- Sketch of how the sprite would look in game
- Colored
- 
...
Sprite size estimates

Small: 16x16 (rocks, bushes, boxes)
Medium: 16x32 or 32x16 (Doors, characters, Book shelf)
Big: 32x32 (Larger bookshelf, Statues etc)

outline should be coloured
colour ramps should have 4 shades per colour 
Less textured
\*elaborate on this

smaller object should be in the same perspective as characters (2:1)

\*add perspective pics
...
- Use a 80x48 pixel rectangle as a general guideline
- Character's heads' are roughly 35 pixels tall
- Black outline (provide RGB values)
- Use predefined palettes for skin tones (Add these)
- Hair colours can include unrealistic colours

(style should be playful, round, colourful)
...
-Black outline (add RGB values)
-32x16 pixels (4 pixel buffer area around)


animation sheet should include:
...
Random stuff
- Yellow as directional colour (yellow butterflies)
- Sepia for splash screens that happened long ago
- splash screens on parchment/frame
